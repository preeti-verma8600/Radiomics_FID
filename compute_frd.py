...
parser.add_argument('--dims', type=int, default=102,
                    help=('Dimensionality of Inception features to use. '
                          'By default, uses pool3 features'))
...
def get_activations(files, model, batch_size=1, dims=102, device='cpu',
                    num_workers=1):
    ...
    for file_path in files:
        if str(file_path).endswith("_mask.png") or str(file_path).endswith("_mask_synth.jpg"):
            ...
            # print('check 1 of mask path------------', mask_paths)
        elif not (str(file_path).endswith("_0001.nii.gz") or str(file_path).endswith("_0001S_grayscale.nii.gz") or str(file_path).endswith("_benign_0.png") or str(file_path).endswith("_img_synth.jpg")):
            mask_paths.append(file_path)
            # print('check 2 of the mask path--------------------------------', mask_paths)
        else:
            image_paths.append(file_path)
            # print('check for the image_paths-----------------------', image_paths)   #



    # print('image_paths--------1', image_paths)
    # print('mask_paths-----------1', mask_paths)
    ...
    for i, image_path in tqdm(enumerate(image_paths)):
        ...
        elif "_0001S_grayscale" in image_name:
            mask_name = image_name.replace("_0001S_grayscale", "")
        ...
        if mask_path.exists():
            ...
            # print(f"Extracted radiomics features for image {i}: {radiomics_features}")
...
def calculate_activation_statistics(files, model, batch_size=1, dims=102,
...
def calculate_frd_given_paths(paths, batch_size, device, dims, num_workers=1):
...
def save_frd_stats(paths, batch_size, device, dims, num_workers=1):
    """Calculates the FID of two paths"""
...
def main():
    ...
    if args.save_stats:
        save_frd_stats(args.path, args.batch_size, device, args.dims, num_workers)
        return
