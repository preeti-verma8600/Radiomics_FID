...
def load_images(directory, normalize=False, split=False, is_split_per_patient=True, reverse_split=False,
    ...
    for count, filename in tqdm(enumerate(os.listdir(directory)), total=limit if limit else len(os.listdir(directory))):
        ...
        if filename.lower().endswith((".png", ".jpg", ".jpeg")) and "_mask" not in filename and "_mask_synth" not in filename:
...
def calculate_fid(
    ...
    if lower_bound and is_only_splitted_loaded:
        ...
        images_1, _ = load_images(directory_1, split=is_split_per_patient, limit=limit, normalize=normalize_images,
                                  is_split_per_patient=is_split_per_patient, reverse_split=reverse_split_ds1,
                                  is_only_splitted_loaded=is_only_splitted_loaded)
        images_2, _ = load_images(directory_2, split=is_split_per_patient, limit=limit, normalize=normalize_images,
                                  is_split_per_patient=is_split_per_patient, reverse_split=reverse_split_ds2,
                                  is_only_splitted_loaded=is_only_splitted_loaded)
